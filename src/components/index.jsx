...
const RandomColor = () => {
    ...
    return (
        <div style={{ height: "100vh", width: '100vw', background: color}}>
            <button onClick={() => setTypeOfColor('hex')}>Create hax color</button>
            <button onClick={() => setTypeOfColor('rbg')}>Create rgb color</button>
            <button onClick={typeOfColor === 'hex'? handleCreateHaxColor: handleCreateRgbColor}>Generate Random Color</button>
        </div>
    )
}
